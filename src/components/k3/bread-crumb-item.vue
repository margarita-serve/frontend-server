<template>
  <li :class="containerClass(item)" v-if="visible(item)">
    <template v-if="!template">
      ...
      <a v-else-if="item.url" :href="item.url || '#'" :class="linkClass(item)" @click="onClick" :target="item.target">
        <span v-if="item.icon" :class="iconClass"></span>
        <span v-if="item.label" class="p-menuitem-text">{{ label(item) }}</span>
      </a>
      <div v-else>
        <span v-if="item.icon" :class="iconClass"></span>
        <span v-if="item.label" class="p-menuitem-text p-plain-text">{{ label(item) }}</span>
      </div>
    </template>
  </li>
</template>
...
const deployID = ref(route.query?.DEID);
const modelID = ref(route.query?.MPID);
const deployName = ref(route.query?.DENAME);
